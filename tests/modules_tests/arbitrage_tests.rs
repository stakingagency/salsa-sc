...
#[test]
fn test_xexchange_arbitrage_delegate() {
    let mut world = setup();
    ...
    set_block_nonce(&mut world, nonce);
    delegate_test(&mut world, DELEGATOR1_ADDRESS_EXPR, &sell_amount, false, true);
    sell_on_xexchange(&mut world, DELEGATOR1_ADDRESS_EXPR, &sell_amount, &rust_biguint!(ONE_EGLD));

    // now the LEGLD price on xexchange is low, so delegate should buy instead of delegating
    nonce += BLOCKS_PER_EPOCH;
    set_block_nonce(&mut world, nonce);
    delegate_test(&mut world, DELEGATOR1_ADDRESS_EXPR, &delegation_amount, false, false);
...
#[test]
fn test_xexchange_arbitrage_undelegate() {
    let mut world = setup();
    ...
    set_block_nonce(&mut world, nonce);
    wrap_egld(&mut world, DELEGATOR1_ADDRESS_EXPR, &buy_amount);
    buy_from_xexchange(&mut world, DELEGATOR1_ADDRESS_EXPR, &buy_amount, &rust_biguint!(ONE_EGLD));
