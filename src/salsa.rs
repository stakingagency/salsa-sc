    ...
    config::ConfigModule + multiversx_sc_modules::default_issue_callbacks::DefaultIssueCallbacksModule
{
    ...
    // Comment
    // I think having the caller as a key for the backup_user_undelegations would help
    // in not having the SC as much time in the "pause" state
    // It will be cleared in the callback, so there is no actual storage increase
    #[endpoint(withdraw)]
    fn withdraw(&self) {
        require!(self.is_state_active(), ERROR_NOT_ACTIVE);
        let caller = self.blockchain().get_caller();
    ...
    fn add_reserve(&self) {
        ...
        self.user_reserves().update(|user_reserves| {
            ...
            for mut user_reserve in user_reserves.into_iter() {
                if user_reserve.address == caller {
                    ...
                    // Comment
                    // It is better to set a value to 0 than to update to 0, as it is less gas expensive
                    // Also, no need to set the value to 0, as BigUint has a default implementation, so we can just clear the value
                    // If the storage is empty, it returns the default value (that's why it usually does not work with custom structs)
                    // So because we want the value and then just clear the storage, it is better to use the take() function
                    let user_reward = self.user_rewards(&caller).take();
        ...
        self.egld_reserve()
            .update(|value| *value += &reserve_amount + &reward);
    ...
    fn update_withdrawn_amount(&self, reserve_withdraw_amount: &BigUint) {
        ...
        if reserve_withdraw_amount <= &withdrawn_amount {
            ...
            // TODO - we should not get here, but we should define a fallback logic
            // maybe save withdrawn_amount and reserve_withdraw_amount variables and put the contract in Inactive state
        }
    }
